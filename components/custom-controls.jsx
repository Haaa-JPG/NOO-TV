...
export default function CustomControls({ videoRef }) {
  ...
  const tapData = useRef({ time: 0, side: null })
  ...
  useEffect(() => {
    if (!v) return
    const add = (e, fn) => v.addEventListener(e, fn)
    const rem = (e, fn) => v.removeEventListener(e, fn)
    const handlers = [
    ...
    handlers.forEach(([e, fn]) => add(e, fn))
    return () => handlers.forEach(([e, fn]) => rem(e, fn))
  }, [v])
  ...
  const doSkip = useCallback((sec) => {
    if (!v) return
    v.currentTime = Math.max(0, Math.min(v.duration || 0, v.currentTime + sec))
  }, [v])

  const doFullscreen = useCallback(() => {
    const el = gestureRef.current?.parentElement
    if (!el) return
  ...
  // ─── Gesture layer (behind controls) ───
  // Single tap = play/pause, Double tap = skip, Long press = 2x
  const getSide = useCallback((x) => {
    const r = gestureRef.current?.getBoundingClientRect()
    if (!r) return 'center'
    const rel = x - r.left
    if (rel < r.width * 0.35) return 'left'
    if (rel > r.width * 0.65) return 'right'
    return 'center'
  }, [])

  const showRippleAnim = useCallback((side) => {
    setRipple(side)
    clearTimeout(rippleTimer.current)
    rippleTimer.current = setTimeout(() => setRipple(null), 350)
  }, [])

  const handleGesture = useCallback((clientX) => {
    const now = Date.now()
    const side = getSide(clientX)
    const prev = tapData.current
    const diff = now - prev.time
    const sameSide = prev.side === side

    clearTimeout(singleTapTimer.current)

    // Long press was active → ignore
    ...
    // Double tap detected
    if (diff < 300 && sameSide && side !== 'center') {
      if (side === 'left') doSkip(-10)
      else doSkip(10)
      showRippleAnim(side)
      tapData.current = { time: 0, side: null }
      return
    }

    // Store first tap
    tapData.current = { time: now, side }

    // Delayed single tap → only play/pause if center
    if (side === 'center') {
      singleTapTimer.current = setTimeout(() => {
        togglePlay()
      }, 280)
    }
  }, [getSide, doSkip, togglePlay, showRippleAnim])

  const showUI = useCallback(() => {
    setVisible(true)
    clearTimeout(hideTimer.current)
    if (playing) hideTimer.current = setTimeout(() => setVisible(false), 3500)
  }, [playing])

  // Long press handlers (on gesture layer only)
  ...
  return (
    <>
      ...
      <div
        ...
        onMouseUp={onUp}
        onTouchStart={onDown}
        onTouchEnd={(e) => { onUp(); handleGesture(e.changedTouches[0].clientX) }}
        onTouchMove={() => clearTimeout(longPressTimer.current)}
        onClick={(e) => handleGesture(e.clientX)}
      />

      {/* CONTROLS LAYER - on top, pointer-events */}
      <div className="absolute inset-0 z-10 pointer-events-none select-none">
        ...
        {/* Speed label */}
        {speedLabel && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none bg-red-500/90 backdrop-blur-sm text-white text-sm font-bold px-4 py-2 rounded-full">
            {speedLabel}
          </div>
        )}
        ...
        <div className={`absolute bottom-0 left-0 right-0 transition-all duration-300 ease-in-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3 pointer-events-none'}`}>
          ...
          <div className="relative px-3 pb-3 pt-10 pointer-events-auto">
            {/* Seek bar */}
            <div
              ref={seekRef}
            ...
            >
              ...
              <div className="absolute w-3 h-3 bg-red-500 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity -translate-x-1/2" style={{ left: `${progress}%` }} />
            </div>

            {/* Bottom row */}
            <div className="flex items-center gap-1">
              <button onClick={(e) => { e.stopPropagation(); togglePlay() }} className="text-white hover:text-red-400 transition p-1.5">
                {playing ? (
              ...
              <button onClick={(e) => { e.stopPropagation(); doSkip(-10) }} className="text-white/60 hover:text-white transition p-1.5">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/><text x="12" y="16" fill="white" fontSize="7" fontWeight="bold" textAnchor="middle">10</text></svg>
              </button>

              <button onClick={(e) => { e.stopPropagation(); doSkip(10) }} className="text-white/60 hover:text-white transition p-1.5">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 5V1l5 5-5 5V7c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6h2c0 4.42-3.58 8-8 8s-8-3.58-8-8 3.58-8 8-8z"/><text x="12" y="16" fill="white" fontSize="7" fontWeight="bold" textAnchor="middle">10</text></svg>
              </button>

              <span className="text-white/70 text-[11px] font-medium whitespace-nowrap select-none px-1">
              ...
              <div
                ...
                onMouseEnter={() => { setShowVolume(true); clearTimeout(volTimer.current) }}
                onMouseLeave={() => { volTimer.current = setTimeout(() => setShowVolume(false), 400) }}
              >
                <button onClick={(e) => { e.stopPropagation(); v && (v.muted = !v.muted) }} className="text-white/60 hover:text-white transition p-1.5">
                  {muted || volume === 0 ? (
              ...
              <button onClick={(e) => { e.stopPropagation(); doFullscreen() }} className="text-white/60 hover:text-white transition p-1.5">
                {fullscreen ? (
