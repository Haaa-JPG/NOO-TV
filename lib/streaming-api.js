...
async function extractPlayback(source, contentId, contentType, sourceContentId) {
  ...
  try {
    ...
    if (!res.ok || !data.success) {
      throw new Error(data.error?.message || `Extract failed: ${res.status}`)
    }

    const { status, playback_url, expires_at, job_id } = data.data

    if (status === 'ready' && playback_url) {
      await pool.query(
      ...
      return { jobId, url: playback_url, expiresAt: expires_at || null }
    }

    if (status === 'processing') {
      return { jobId: job_id || jobId, url: null, expiresAt: null, processing: true }
    }

    throw new Error('Unexpected extract response status')
  } catch (err) {
    await pool.query(
...
async function refreshPlayback(source, contentId, contentType, sourceContentId) {
  ...
  try {
    ...
    const { job_id, status: jobStatus } = data.data

    return { jobId: job_id || jobId, url: null, expiresAt: null, jobStatus }
  } catch (err) {
    await pool.query(
      `SELECT public.fail_streaming_job($1, $2)`,
      [jobId, err.message]
