...
import { Heart, Star, ArrowRight, MessageCircle, Play, ListVideo, ThumbsUp, ThumbsDown, ChevronDown, Disc, Clock, Bookmark, Share2, Calendar, Eye, Clapperboard } from 'lucide-react'
...
export default function WatchSeries() {
    ...
    if (currentUser) {
      ...
      if (targetEpisodeId) {
        for (const s of withEpisodes) {
          ...
          if (found) { setSelectedSeason(s); setSelectedEpisode(found); setResumeTime(progressMap[found.id]?.time || 0); return }
        }
      }
      if (historyList.length > 0) {
        ...
        for (const s of withEpisodes) {
          ...
          if (found) { setSelectedSeason(s); setSelectedEpisode(found); setResumeTime(historyList[0].watched_time || 0); return }
        }
      }
    }
    const firstSeason = withEpisodes.find((s) => s.episodes.length > 0)
    if (firstSeason) { setSelectedSeason(firstSeason); setSelectedEpisode(firstSeason.episodes[0]) }
  }
  ...
  const startEpisode = async (episode) => {
    ...
    if (user) await supabase.from('watch_history').insert({ user_id: user.id, content_id: episode.id, content_type: 'episode', episode_id: episode.id, duration: episode.duration || 0 })
    let sessionId = localStorage.getItem('nootv_session')
    if (!sessionId) { sessionId = crypto.randomUUID(); localStorage.setItem('nootv_session', sessionId) }
    const { data: existing } = await supabase.from('view_tracking').select('id').eq('session_id', sessionId).eq('content_type', 'episode').eq('content_id', episode.id).maybeSingle()
    if (!existing) { await supabase.from('view_tracking').insert({ session_id: sessionId, content_type: 'episode', content_id: episode.id }); await supabase.rpc('increment_episode_views', { ep_id: episode.id }) }
  ...
  const handleVideoProgress = async (currentTime, duration, ended = false) => {
    ...
    const { data: existing } = await supabase.from('watch_history').select('id').eq('user_id', user.id).eq('episode_id', epId).maybeSingle()
    if (existing) await supabase.from('watch_history').update({ watched_time: watchedTime, duration, watched_at: new Date().toISOString() }).eq('id', existing.id)
    else await supabase.from('watch_history').insert({ user_id: user.id, content_id: epId, content_type: 'episode', episode_id: epId, watched_time: watchedTime, duration })
    setEpisodeProgress(prev => ({ ...prev, [epId]: { time: watchedTime, duration } }))
  }

  const totalEpisodes = () => seasons.reduce((sum, s) => sum + (s.episodes?.length || 0), 0)

  const loadEpisodeLikeForEpisode = async (episodeId) => {
    if (!episodeId) { setEpisodeLike(null); setEpisodeLikeCounts({ likes: 0, dislikes: 0 }); return }
    const { data: allLikes } = await supabase.from('episode_likes').select('is_like').eq('episode_id', episodeId)
    ...
    const { data } = await supabase.from('episode_likes').select('is_like').eq('user_id', user.id).eq('episode_id', episodeId).maybeSingle()
    setEpisodeLike(data?.is_like ?? null)
  }

  useEffect(() => { if (selectedEpisode) loadEpisodeLikeForEpisode(selectedEpisode.id) }, [selectedEpisode])

  const toggleEpisodeLike = async (isLike) => {
    if (!user) { router.push('/auth'); return }
    if (!selectedEpisode) return
    const { data: existing } = await supabase.from('episode_likes').select('id, is_like').eq('user_id', user.id).eq('episode_id', selectedEpisode.id).maybeSingle()
    if (existing) {
      if (existing.is_like === isLike) { await supabase.from('episode_likes').delete().eq('id', existing.id); setEpisodeLike(null) }
      else { await supabase.from('episode_likes').update({ is_like: isLike }).eq('id', existing.id); setEpisodeLike(isLike) }
    } else {
      await supabase.from('episode_likes').insert({ user_id: user.id, episode_id: selectedEpisode.id, is_like: isLike })
  ...
  return (
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="relative container mx-auto px-4 sm:px-6 pb-12 sm:pb-20 pt-32">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4">
              {selectedEpisode && (selectedEpisode.active_stream_url || selectedEpisode.embed_url) ? (
                <button
      ...
      {/* ===== VIDEO SECTION ===== */}
      {selectedEpisode && (selectedEpisode.active_stream_url || selectedEpisode.embed_url) && (
        <section id="video-section" className="container mx-auto px-4 sm:px-6 -mt-10 sm:-mt-20 relative z-10">
          <div className="relative bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10" style={{ aspectRatio: '16 / 9' }}>
            <VideoPlayer
              url={selectedEpisode.embed_url}
              activeStreamUrl={selectedEpisode.active_stream_url}
          ...
          <div className="flex items-center justify-between mt-4 px-2">
            <div className="flex items-center gap-3">
              <span className="text-white/80 font-semibold text-sm sm:text-base">الحلقة {selectedEpisode.episode_number}: {selectedEpisode.title || `الحلقة ${selectedEpisode.episode_number}`}</span>
              <span className="text-white/40 text-sm">{selectedEpisode.views || 0} مشاهدة</span>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => toggleEpisodeLike(true)} className={`w-9 h-9 rounded-full flex items-center justify-center transition ${episodeLike === true ? 'bg-emerald-500/20 text-emerald-500' : 'bg-white/10 text-white/50 hover:bg-white/20'}`}>
                <ThumbsUp className="w-4 h-4" />
              </button>
              <span className="text-xs text-white/40">{episodeLikeCounts.likes}</span>
              <button onClick={() => toggleEpisodeLike(false)} className={`w-9 h-9 rounded-full flex items-center justify-center transition ${episodeLike === false ? 'bg-red-500/20 text-red-500' : 'bg-white/10 text-white/50 hover:bg-white/20'}`}>
                <ThumbsDown className="w-4 h-4" />
              </button>
              <span className="text-xs text-white/40">{episodeLikeCounts.dislikes}</span>
            </div>
          </div>
        </section>
      )}
                  ...
                  selectedSeason.episodes.map((ep) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative rounded-xl overflow-hidden border transition-all duration-300 text-right hover:shadow-xl ${
                          isActive
                            ? 'border-primary ring-2 ring-primary/30 bg-primary/5 shadow-lg shadow-primary/10'
                      ...
                      >
                        ...
                        <div className="relative aspect-video overflow-hidden bg-white/5">
                          ...
                          {/* Active indicator */}
                          {isActive && (
                            <div className="absolute top-3 right-3 w-3 h-3 rounded-full bg-primary animate-pulse shadow-lg shadow-primary/50" />
                          )}
                        </div>
            ...
            <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-6 sm:p-8">
              ...
              {user ? (
                <form onSubmit={handleComment} className="mb-6 sm:mb-8">
                  ...
                  />
                  <Button type="submit" className="mt-3 bg-primary hover:bg-primary/90 px-6">إرسال</Button>
                </form>
              ) : (
