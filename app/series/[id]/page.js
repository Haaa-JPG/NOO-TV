...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const { rows } = await client.query(
      'SELECT title, description, thumbnail, banner, average_rating, views, is_translated, is_dubbed FROM series WHERE id = $1',
      [id]
    )
    if (rows.length === 0) return { title: 'مسلسل غير موجود' }
    const s = rows[0]
    const title = s.title
    const desc = s.description || `شاهد ${title} مجاناً بجودة عالية على NOO TV`
    const img = s.banner || s.thumbnail
    const url = `https://noo-tv.vercel.app/series/${id}`

    return {
      title,
      description: desc,
      keywords: [title, 'مسلسل', 'مشاهدة مجاناً', 'مترجم', 'مدبلج', 'NOO TV', 'نوفا'],
      openGraph: {
        title: `${title} | NOO TV`,
        description: desc,
        url,
...
export default async function SeriesDetailPage({ params }) {
  ...
  try {
    ...
    if (seriesRows.length > 0) {
      ...
      const { rows: seasonsRows } = await client.query(
        'SELECT * FROM seasons WHERE series_id = $1 AND is_active = true ORDER BY season_number ASC',
        [id]
      )
      seasons = seasonsRows
      if (seasons.length > 0) {
        ...
        const { rows: epsRows } = await client.query(
          'SELECT COUNT(*)::int as count FROM episodes WHERE season_id = ANY($1) AND is_active = true',
          [seasonIds]
        )
  ...
  const jsonLd = {
    ...
    inLanguage: series.is_translated ? ['ar'] : undefined,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
